...
public class DivConstantInversedInstruction : IInstruction
{
    private readonly int _constant;
    private readonly string _register;
    private readonly string _registerResult;

    public DivConstantInversedInstruction(int constant, string register, string registerResult)
    {
        _constant = constant;
        _register = register;
    ...
    public void Execute(Interpreter interpreter)
    {
        ...
        if (a == 0)
        {
            interpreter.RaiseException("Division by zero");
            return;
        }
